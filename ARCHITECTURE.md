...
- **Frontend — Vercel (static site hosting for `client/`)**
    ...
    - Environment variables (set in Vercel project Settings → Environment Variables): set `VITE_API_BASE` to the actual backend API host. The Streamlit monitor URL is not the API; use the host where the Express API is deployed.
